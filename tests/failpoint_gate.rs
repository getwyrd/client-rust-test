//! Failpoint-driven proof of **finding 2**: after a 2PC commit fails at
//! prewrite, `Transaction::rollback()` clears the already-placed prewrite locks
//! in **optimistic** mode (`BatchRollback`) but **not** in **pessimistic** mode
//! (`PessimisticRollback` only removes `LockType::Pessimistic` locks, silently
//! skipping the prewritten Put/Delete 2PC locks — and still returns `Ok`).
//!
...
//!
//! Applies to client-rust independent of the #543 fix (that fix touches lock
//! *resolution*, not `Committer::rollback`), so this reproduces on both
//! `e53837d` and the fix branch.
...
async fn d7_pessimistic_rollback_leaves_prewrite_locks() {
    ...
    // The baseline: optimistic BatchRollback cleared the prewrite lock.
    assert_eq!(
        opt_locks, 0,
        "optimistic rollback (BatchRollback) must clear the prewrite lock"
    );
    // The finding, proven: pessimistic rollback reported Ok yet left the
    // prewrite lock behind (PessimisticRollback skips non-pessimistic locks).
    assert_eq!(
        pess_locks, 1,
        "FINDING 2: pessimistic rollback() returned Ok but left the prewrite lock in place \
         (see findings/pessimistic-rollback-leaves-prewrite-locks.md). If this is now 0, \
         has client-rust switched the terminal pessimistic rollback to BatchRollback?"
    );
}
